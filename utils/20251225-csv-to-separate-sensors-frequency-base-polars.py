...
3. Output clean CSV: Timestamp + data columns only (no conflict columns)
"""
...
def calculate_smoothness_score(
    ...
    # Score 1: Deviation from local mean
    if local_std > 1e-9:
        deviation_score = abs(value - local_mean) / local_std
    else:
        deviation_score = abs(value - local_mean)
    
    # Score 2: Curvature (second derivative approximation)
    left_val = None
    right_val = None
...
def optimize_for_smoothness(
    ...
    # Find indices where we have both base and alt values (conflicts)
    ...
    for idx in conflict_indices:
        ...
        # Calculate expected value from trend
        expected = calculate_local_expected(result, idx, window)
        
        if expected is None:
            continue
        
        # Calculate smoothness scores (lower = better)
        ...
        # Also consider distance from expected
...
def merge_sensor_files(
    ...
    for col in sorted(all_data_columns):
        ...
        # Collect all (timestamp, value) pairs
        ...
        # Count frequency of each (timestamp, value)
        freq_counts = col_data.group_by(["Timestamp", col]).len().rename({"len": "__freq__"})
        
        # Rank by frequency within each timestamp
        ...
        if max_rank > 1 and is_numeric:
            ...
            # Convert to numpy for smoothness optimization
            ...
            # No conflicts or not numeric - just use base values
...
def print_summary(all_stats: list[SensorStats]) -> None:
    ...
    print(f"\n{'Sensor':<40} {'Rows':<12} {'Conflicts':<12} {'Smoothness Swaps':<20}")
    print("-" * 100)
    ...
    for s in all_stats:
        ...
        print(f"{name:<40} {s.rows:<12} {conflicts:<12} {swaps:<20}")
    ...
    print("-" * 100)
    print(f"{'TOTAL':<40} {total_rows:<12} {total_conflicts:<12} {total_swaps:<20}")
...
def main():
    ...
    parser.add_argument(
        "output_folder", 
        type=str, 
        help="Folder to save merged CSV files"
    )
    parser.add_argument(
